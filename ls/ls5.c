...
void list_name(char* path) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // printf("%s\n", namelist[i]->d_name);
        // print_aligned(namelist[i]->d_name,12);
        free(namelist[i]);
        // if (i % 10 == 9)
        //    printf("\n");
    }
...
void list_l(char* path) {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (flag_i && flag_s) {
            ...
            printf("%-8ld%c%s %2ld %s %s%6ld %s ", namelist[i]->d_ino, c, strb,
                   (long)sb->st_nlink, get_uid(sb), get_gid(sb),
...
//                 struct file* temp = (struct file*)malloc(sizeof(struct
//                 file)); temp->time = b[k]->time; b[k]->time = b[k + 1]->time;
//                 b[k]->time = temp->time;
//                 free(temp);
//             }
//         }
...
// }
int list_t(const struct dirent** a, const struct dirent** b) {
    // const char* path_a = get_cwd(a);
    // const char* path_b = get_cwd(b);
    ...
    // printf("File A: %s, Time A: %ld\n", path_a, sb_a->st_mtime);
    // printf("File B: %s, Time B: %ld\n", path_b, sb_b->st_mtime);
...
// printf("%s%s", color, namelist[i]->d_name);  // 带颜色打印文件名
void list_R(char* path) {}